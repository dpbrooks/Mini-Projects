/*
  Filename   : List.hpp
  Author(s)  : William Killian, Gary M. Zoppetti, Dylan Brooks
  Course     : CSCI 362-01
  Assignment : List
...
public: /* should be private, but public for testing */
  ...
  transfer (iterator pos, iterator first, iterator last)
  {
    ...
    if (std::distance(first, last) != 0) {
      first.m_nodePtr->unhook_range(first.m_nodePtr, (--last).m_nodePtr);
...
public:
  ...
  erase (iterator pos)
  {
    ...
    iter.m_nodePtr->unhook();
    delete iter.m_nodePtr;
    --m_size;
    return pos;
  }
